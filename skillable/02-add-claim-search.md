...
1. [] Click on the model and then click **Use this model**.
...
1. [] In VS Code, navigate to **infra/data/sample-data/claims.json**.

1. [] Notice the structure - each claim has:
...
1. [] This data will be indexed into Azure AI Search when you run the agent.

### Step 4: Configure Azure AI Search Credentials

Now let's add your Azure AI Search credentials to the project.

1. [] Open **env/.env.local** in VS Code.

1. [] Find the Azure AI Search section and replace it with the text below:

    ```bash
    # Azure AI Search
    AZURE_AI_SEARCH_ENDPOINT=@lab.Variable(SearchURL)
    ```

1. [] Open **env/.env.local.users** in VS Code.

1. [] Find the **SECRET_AZURE_AI_SEARCH_API_KEY=** line and replace it with the text below:
...
1. [] In VS Code, create a new folder under **src** and call it +++Services+++.

1. [] Create a new file under **src/Services/** and call it +++KnowledgeBaseService.cs+++ and add the complete implementation:
...
1. [] Create a new file in the folder **src/Plugins/** and name it +++ClaimsPlugin.cs+++, then add the complete implementation:
...
1. [] Open **Program.cs** in the root of the project.

1. [] At the top with other using statements, add:
...
1. [] Open **src/Agent/ZavaInsuranceAgent.cs**.

1. [] Add the following using statements at the top:
...
1. [] Select **(Preview) Debug in Copilot (Edge)** if prompted.
...
1. [] A browser window will open with Microsoft 365 Copilot. Your agent should already be installed from the previous lab.

1. [] **Verify in Azure Portal** (optional but recommended):
...
1. [] In Microsoft 365 Copilot, try a more specific search: +++Find claims in the South region+++

1. [] Try: +++Show me auto claims with medium severity+++

### Step 3: Test Claim Details

1. [] Try: +++Get details for claim CLM-2025-001007+++

    >[!Note] The agent should use **GetClaimDetails** and return detailed information. Note that we'll continue adding more data in future labs that will enhance the responses such as showing policy or claim history in claim details.

1. [] Try another claim: +++Show me information about claim CLM-2025-001003+++
